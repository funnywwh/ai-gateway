	...
	return []adminRoute{
		...
		},
		{
			...
			Body: []adminField{
				...
				structuredField("skill_ids", "array", "要加载的私有技能 id 列表（来自技能库；指令会随系统提示词发给模型）", idArraySchema(), []any{}),
			},
			NoTool: chatNoTool,
		},
		{
		...
		},
		{
			...
			Body: []adminField{
				...
				structuredField("skill_ids", "array", "替换要加载的技能 id 列表；传空数组表示清空", idArraySchema(), []any{}),
			},
			NoTool: chatNoTool,
		},
		{
