...
type ChatArtifactStore interface {
	...
	// SetChatArtifactBridgeToken stores (or clears) the handshake secret of one preview. It is
	// a separate call rather than a field of Upsert because the upsert is keyed by
	// (session, key) and its caller does not own the row's identity.
...
type chatArtifactRequest struct {
	...
	// BridgeToken is the handshake secret, generated by the console and echoed on every
	// response so both ends hold the same value without a second request.
	//
	// It has to come from the client because of where it can safely live. The console cannot
	// read it out of the frame afterwards: a sandboxed document is an opaque origin, so
	// `frame.contentDocument` is null for the parent (that is exactly how the preview lost its
	// bridge the first time). Putting it in the response *header* would work but needs an extra
	// request, and putting it in the URL would hand it to the page. Passing it in the request
	// body keeps it in exactly two places — this upload and the injected script tag — and the
	// model's page can read neither.
...
func (s *Server) handleAdminChatPutArtifact(w http.ResponseWriter, r *http.Request) {
	...
	if body.Bridge {
		scope = stored.ID
		bridgeToken = strings.TrimSpace(body.BridgeToken)
		if bridgeToken == "" {
			// Without it the injected script could not tell its host apart from a nested frame
			// that offers a port of its own, so an interactive preview without one is refused
			// rather than served with a channel nobody can authenticate.
			writeAPIError(w, domain.ErrInvalidRequest(
				"an interactive preview needs bridge_token: it is the handshake the page answers with"))
			return
		}
		if err := artifacts.SetChatArtifactBridgeToken(r.Context(), stored.ID, bridgeToken); err != nil {
			writeAPIError(w, toAPIError(err))
			return
		}
	} else {
		// A read-only re-preview of a block that was interactive must not keep a live token.
		if err := artifacts.SetChatArtifactBridgeToken(r.Context(), stored.ID, ""); err != nil {
			s.deps.Log.Warn("clearing a stale bridge token failed", "err", err, "artifact", stored.ID)
		}
	}
...
func (s *Server) handleAdminChatTicket(w http.ResponseWriter, r *http.Request) {
	...
	if body.Bridge {
		...
		if bridgeToken == "" {
			// The block was first stored as a read-only preview, so it has no token yet; one is
			// issued here so that "re-open this page and interact with it" does not require
			// re-uploading the payload the console already has.
			bridgeToken = newBridgeToken()
		}
		if err := artifacts.SetChatArtifactBridgeToken(r.Context(), id, bridgeToken); err != nil {
			writeAPIError(w, toAPIError(err))
			return
		}
		scope = stored.ID
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"expires_at": expires, "bridge": body.Bridge, "bridge_token": bridgeToken,
	})
}
...
func (s *Server) handleAdminChatArtifact(w http.ResponseWriter, r *http.Request) {
	...
	if interactive {
		body = []byte(injectUIBridge(artifact.Body, artifact.BridgeToken, nonce))
...
// newBridgeToken is the handshake secret used when a preview becomes interactive without a
// fresh upload (the console re-opened a block it had stored read-only). Normally the console
// generates it and sends it with the upload; see chatArtifactRequest.BridgeToken for why it
// cannot be minted per response.
