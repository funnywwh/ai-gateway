// Package tenancy owns OS-facing provisioning and deployment rendering. It is
// the only dshgw package allowed to invoke useradd/systemctl/nginx.
package tenancy
...
func RenderTenantArtifacts(cfg *config.Config, t registry.Tenant, key string, models []string, opt TenantOptions, now time.Time) ([]Artifact, error) {
	...
	env := []byte(fmt.Sprintf("DSH_PORT=%d\nDSH_HOME=%s\nHOME=%s\nDSHGW_DSH_ANCHOR=%s\n", t.WorkerPort, quoteEnv(t.DshHome), quoteEnv(t.Workspace), quoteEnv(filepath.Join(cfg.Dsh.CurrentLink, "package.json"))))
	return []Artifact{
		...
		{filepath.Join(cfg.Deploy.TenantConfigRoot, t.Name, "tenant.env"), env, 0o640},
...
func quoteEnv(v string) string { return strconv.Quote(v) }

func RenderNginx(cfg *config.Config, tenants []registry.Tenant) (map[string][]byte, error) {
	if cfg.TLS.Certificate == "" || cfg.TLS.CertificateKey == "" {
		return nil, errors.New("tls.certificate and tls.certificate_key are required")
	}
	files := map[string][]byte{"00-common.conf": []byte("map $http_upgrade $dshgw_connection_upgrade {\n    default upgrade;\n    '' close;\n}\n")}
	files["01-portal.conf"] = []byte(renderServer(cfg, cfg.PortalPort, "dshgw portal"))
	sort.Slice(tenants, func(i, j int) bool { return tenants[i].Name < tenants[j].Name })
	for _, t := range tenants {
		files["tenant-"+t.Name+".conf"] = []byte(renderServer(cfg, t.PublicPort, "dshgw tenant "+t.Name))
	}
	return files, nil
}
func renderServer(cfg *config.Config, port int, label string) string {
	listen := net.JoinHostPort(cfg.Deploy.PublicListen, strconv.Itoa(port))
	return fmt.Sprintf(`# %s — generated by dshgw; do not edit
server {
    listen %s ssl;
    server_name %s;
    ssl_certificate %s;
    ...
    location / {
        ...
        proxy_set_header %s $server_port;
        ...
        proxy_set_header Connection $dshgw_connection_upgrade;
        proxy_buffering off;
...
`, label, listen, cfg.PublicHost, cfg.TLS.Certificate, cfg.TLS.CertificateKey, cfg.Listen, cfg.EdgePortHeader)
}
