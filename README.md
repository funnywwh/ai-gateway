...
| `docs/chat.md` | 控制台智能问答与技能库：计费与隐私口径、按 MCP 令牌 scope 决定的权限边界、Markdown/图表/HTML5 预览、**内联表单（气泡内渲染 → 提交 → 模型原地更新）**、**可交互沙箱页面（生成表单 → 用户提交 → 模型继续执行）**、技能私有性、中断恢复、配置与排障 | **已实现（M32 + M34 + M35）** |
