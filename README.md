...
M81 把 `user` 档再收一档：**每条** user 消息最多保留前 `recording.input_max_chars` 个字符（默认 100，
`0` = 不限），超出部分丢弃并在录制文档里标 `input_truncated`/`input_max_chars`，消息里的非文本部分
（图片 base64 等）不再落库、只进 `omitted` 计数；`full`/`metadata`/`off` 三档与控制台流量不受影响，
历史行不重填。理由是 M23 之后日志里仍是一份完整提问副本，而日志全管理员可读、会被备份带走；
控制台详情会明写「已截断：每条用户消息只留前 N 字符」，免得被截断的提问读起来像全文。
...
| `docs/request-log.md` | 请求日志：录制通道矩阵（含 `user` 档每条消息的字符上限）、身份维度（客户端/模型/工作区/会话/标题 + 用户（账户）/API Key + 供应商/上游模型/路由路线）、消耗口径、查询与统计（含列表底部的本页汇总行）、保留期与写入兜底 | **已实现（M27、M29、M30、M53、M78、M81）** |
