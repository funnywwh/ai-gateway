...
| `docs/design/` | 每个里程碑的设计文档（接口、数据流、决策、异常、测试策略、实现差异） | M0–M29 全部产出（M28 的决策记在 `docs/api-providers.md` 与 `docs/TODO.md`） |
...
| `docs/request-log.md` | 请求日志：录制通道矩阵、身份维度（客户端/模型/工作区/会话/标题）、消耗口径、查询与统计（含列表底部的本页汇总行）、保留期与写入兜底 | **已实现（M27、M29）** |
