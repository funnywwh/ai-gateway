...
写权限是会话级开关，模型不能改；签发凭据、权限变更、备份恢复、hooks、供应商凭据等危险操作即使开了
写权限也只能由人在控制台页面完成。回答支持 Markdown、图表（`chart` JSON → 本地 SVG，可导出 CSV/SVG）
与 HTML5 预览（短时票据 + 强制沙箱，默认禁止外部资源与网络）。
...
| `docs/design/` | 每个里程碑的设计文档（接口、数据流、决策、异常、测试策略、实现差异） | M0–M32 全部产出（M28 的决策记在 `docs/api-providers.md` 与 `docs/TODO.md`） |
...
| `docs/chat.md` | 控制台智能问答与技能库：计费与隐私口径、写权限与危险操作边界、Markdown/图表/HTML5 预览、技能私有性、中断恢复、配置与排障 | **已实现（M32）** |
