...
- **组织架构页**（`#/org`，侧边栏「访问控制」组）——M72 起它同时是**账号（人员）管理的主界面**：
  - 左/右两栏：组织树 + 选中节点的**人员列表**。人员就是账号；每行显示账号名、DSH 状态、飞书名、
    Key 数与 `#id`，行可**展开**成该账号的详情：
    ...
    - **启用/停用 DSH**：见下一条；
    - **分配组织**：整表替换该账号的 `org_node_ids`（与账户页的编辑是同一件事）。
  ...
  - **未归属账户**：人员列表顶部有一行合成行「未归属账户 N 个」（不在任何节点下的账号）。
    它是视图不是节点：点开只列出这些账号，**成员勾选在它下面不可编辑**（没有节点可写）。
  - 成员分配：在详情卡里勾选账号后「保存成员」（整表替换该节点的成员）。勾选**只在"全部账户"
    范围下可编辑**——按部门/未归属过滤时列表是视图而非成员全集，此时整表替换会误删别人的归属。
...
- **账户页**：「所属组织」列与**按组织筛选**（可选是否包含子节点）；编辑账户时可设置
  `org_node_ids`（逗号分隔的节点 id，整表替换该账号的归属）。DSH 列在 M72 起区分三态
...
| 组织页里"保存成员"是灰的 | 当前不是"全部账户"范围（选了某个部门或「未归属账户」）。过滤态下的列表是视图而非成员全集，整表替换会误删别人的归属，所以只能在全部账户下编辑 |
