...
| 通道 | 开关 | 默认 | 内容 |
|---|---|---|---|
| 输入 | `recording.record_input`（可被 Key 覆盖） | `user` | `full` 整份正文（**保留全部**）/ `user` 只留最新一条合格用户输入的**纯文本** / `metadata` 不落正文 / `off` 不落正文 |
| 输入长度阈值 | `recording.input_max_chars` | 100 | `user` 档只保留**不超过**它的 user 消息；`0` = 不过滤（全部有文本的 user 消息拼接） |
...
**输入口径（M82）**：`request_json` 就是**一段纯文本**——不是 JSON 文档、没有 `type`/`role`/`content`
结构、没有计数与阈值字段。规则：

- **只留最新一条合格的** user 消息：从末尾往回找，取第一条**有文本**（去掉首尾空白后非空）且**不超过**
  `recording.input_max_chars`（默认 100，按字符计、中文算 1；恰好 100 **保留**）的消息。
  空的（空文本、纯空白、只有图片、content 读不懂）与超长的都会被**跳过**并继续往前找；一条都不符合 →
  正文为空（v4.3.2 起的口径：以前只看字面最后一条，末条为空或超长就会把整行正文丢掉）。
- 保留就**整条原样保留**（多条文本 part 用换行连接），不再截断——日志里不会出现「看起来完整、其实是前 100
  字符」的假头。
- 末条 ≥ 阈值、末条 content 读不懂、或这次请求根本没有 user 消息 → 该行正文为**空**；
  行本身照写（状态、体积、身份维度都在）。因此 `input_recorded=false` 且该行 `record_input_mode=user`
  的含义是「策略要记，但没有可保留的用户输入」——**看不出具体原因**（这是有意的收窄）。
- `recording.input_max_chars: 0` = **不过滤**：把所有**有文本**的 user 消息按顺序换行拼接成一段（仍然只是文本）。
- **要保留全部（原样字节）就把该 Key 的输入录制切成 `full`**：该档存整份请求正文的 JSON，含工具定义、
  工具输出、系统指令与图片，**不受**上述阈值、不受「只留一条」、不受「纯文本」三条限制，
  仍只受 `recording.max_bytes` 兜底。`metadata`/`off` 不落正文；控制台智能问答流量服务端强制 `off`。
