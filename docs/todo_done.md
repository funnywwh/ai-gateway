...
- [x] **现场与根因**：key 127（`dshgw-dsh-chengjinfeng-8e59`）14:00 前 352 行 0 空 → 阈值生效后 71 行
      **71 空**；对照 key 24（提问 2–44 字符）376 行仅 5 空。读它的会话文件（只暴露长度与开头标记）：
      ...
      确认是策略按设计生效而非错误路径。
- [x] **口径改为按标记判定**：`boilerplatePrefixes`（复用 `dimensions.go` 的常量 + `<system-reminder>`、
      `<skills_instructions>`、两个标题提示词）——以这些开头（`TrimLeft` 后）的消息一律跳过；从末尾往回找
      第一条「有文本、非样板、不超过上限」的消息，整条原样保留。上限默认 **100 → 2000**，角色从判定器变成
      防呆（`0` = 不限长度；样板仍然跳过，因为 `0` 不是「什么都记」）。
- [x] **测试**：`internal/responses` 新增现场回归（117 字符提问 + 542 字符样板 → 记提问）、八种客户端样板各
      一条、超长人话回退、`0` 时样板仍跳过、只在中间提到标记的人话不被误杀；`internal/httpapi` 端到端加
      「117 字符提问在样板之后必须落库」与「只有样板 → 空正文」；既有断言全部保持。测试新增 `postAndLog`
      辅助（排空响应 + 轮询等行）——大响应下「关闭 body 不读」会让断言与 handler 收尾抢跑，这是测试侧的
      抢跑而不是产品缺陷（`persist` 在响应写完后无条件执行）。
- [x] **文案/文档**：控制台「未保留：只有样板或超长用户消息」；`docs/request-log.md`（含为什么必须用标记，
      附本次现场数字）、`config.example.yaml`、`docs/mcp.md`、`README.md`、`internal/config`、
