...
>
> **2026-09-22 追补（M76）**：点「退出」后网关在这个 POST 里**同步**做完"强制卸载该账号的挂载 →
> 最后强制停掉它的 dsh"才回 303，所以按钮的「退出中…」覆盖整个过程（正常 <1s，最坏 55s）。
> 客户端不需要改动；顺序与取舍见 [M76](m76-dsh-exit-force-teardown.md)。
