...
7. **运行中的网关仍需重启才能生效**：deadline 改动在网关二进制里，沙箱内无法给宿主的实例发信号，
   因此该实例仍跑旧二进制（10s deadline + 旧探测）。重启命令：在宿主终端执行
   `./scripts/local-run.sh restart`。本里程碑的所有实测都在隔离实例（新二进制）上完成。

8. **UI 侧的配套改动（本里程碑的直接后果，非额外功能）**：探测从 ~0.6s 变为 ~16s，而控制台的
   「探测」按钮**没有任何加载态**——点击后十几秒界面毫无反应，与"卡死"无法区分；且 `probe()` 原本
   **没有错误处理**，请求一旦抛错就是未捕获的 promise，用户连失败都看不到。因此一并补上：

   - `ui.js` 新增可复用的 `withBusy(button, label, action)`：执行期间禁用按钮、把文案换成
     spinner + `探测中…`、并让秒数递增（16 秒的等待里"还在走"比静止的转圈更可信），结束时还原
     文案与可点击状态；动作抛错时同样还原。
   - `toast()` 增加 `{sticky}` 选项（返回节点，由调用方移除），供"新建供应商后自动探测"这条
     没有按钮的路径显示持续进度。
   - `probe()` 补上 `try/catch`，失败也给出提示；探测结束后刷新列表，让 `health`/`last_error`
     的变化立即可见。

   验证方式（都实际执行过）：用 node + 最小 DOM 桩对 `withBusy` 跑了 10 项行为断言
   （执行期间禁用/文案/spinner、结束还原、返回值透传、异常后仍还原、无按钮路径直通），
   并单独确认秒数在动作期间持续跳动（采样到 `探测中… 0s` → `探测中… 2s`）；
   再在隔离实例上抓取资源，核对新二进制提供 `.spinner`/`withBusy`/`探测中` 而运行中的旧实例仍为旧资源。
