...
上线后又修了一个**竞态**：控制台先 `append(frame)`、再 `createUIPort(...)`（监听器在里面注册）。
iframe 一进 DOM 就开始加载，注入脚本可能在监听器存在之前发出问候——那条消息被浏览器直接丢掉，
而脚本只发一次，于是永远握不上手，工具栏 3 秒后变成「不可交互」。

两处修：监听器先注册再挂载 frame；脚本的问候重发（600ms × 最多 6 次，收到 port 即停）。
重发是幂等的（宿主只认第一条 hello），因此对安全模型没有影响。
