...
| D15 界面交互 | 部门树的每行一个复选框（含合成根）。工具条给「全选 / 清空 / 连同子部门」三个**显式**动作：「连同子部门」是一次性动作，把已勾选部门的子孙也勾上，**不做**隐式的父子联动（隐式作用域正是操作员最容易被骗的地方）。范围外的人在右侧列表置灰并注明原因；选择变化后（防抖 300 ms）重新取预览——目录走 60 秒缓存，代价可忽略，换来的是"确认框里的数字与服务端计划永远一致"。 |
