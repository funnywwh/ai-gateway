...
## 12. 部门选择（v2.11 追加，2026-09-21）

需求（用户原话）：「用户可以选择同步哪些部门」。M70 第一版是"一键全量"：所有缺失部门都建、所有匹配上的
人都合并。真机一跑（22 个部门 / 90 人）就能看出问题——操作员往往只想先同步一个部门试水，
或者只维护其中几个部门，而"全量"是不可撤销的第一刀。

**作用域模型**（都是"这次写哪些"，不改变匹配规则本身）：

| 决策 | 内容 |
|---|---|
| D8 选择的单位 | 一个**部门 id 集合**。飞书的虚拟根 `"0"` 也是可选单位（界面上的合成根「飞书根组织」）：勾上它表示**公司层人员**（没有任何部门的人）纳入本次同步。 |
| D9 父链自动补建 | 勾了 `平台组` 而没勾 `研发部` 时，`研发部` 节点**依然会被创建**（否则子节点无处安放），但它的**人员不在范围内**。界面把这种节点标成「为层级补建」，勾选状态保持未勾。 |
| D10 人员的判定 | 用**人员自己的部门**是否被勾选；祖先补建**不**扩大人员范围。没有部门的人只由根节点 `"0"` 决定。 |
| D11 同名先到先得仍在全量上判定 | 匹配（三条通道、同名争用）依旧对**整个目录**计算，与勾选无关。因此同一份目录无论勾哪些部门，某个人"该合并到哪个账户"都是一致的；勾选只决定**这次写哪些人**。否则勾选顺序会悄悄改变合并结果。 |
| D12 空选 | 界面在没勾任何部门时禁用「同步」；接口收到**空数组**回答 400（"要么勾至少一个部门，要么不传这个字段表示全量"），静默什么都不做更糟。 |
| D13 不传 = 全量 | `department_ids` 缺省（以及预览不传 `departments`）仍是 M70 的全量语义，MCP/脚本路径不受影响。 |
| D14 未知 id | 目录里不存在的 id（比如刚被删）被丢弃并回报（`unknown_department_ids` + warning）；**全部**都不认识才 400。 |
| D15 界面交互 | 部门树的每行一个复选框（含合成根）。工具条给「全选 / 清空」两个动作。**勾选/取消一个部门会连同它的整棵子树一起**（见下一条的修订）。范围外的人在右侧列表置灰并注明原因；选择变化后（防抖 300 ms）重新取预览——目录走 60 秒缓存，代价可忽略，换来的是"确认框里的数字与服务端计划永远一致"。 |
| D15a 父子联动（2026-09-21 修订） | 初版刻意**不做**联动，只给一个「连同子部门」的一次性按钮，理由是"隐式作用域最容易被骗"。用户明确要求改成联动：**勾选父部门 → 子孙全部勾选；取消 → 子孙全部取消**（对任意一行都成立：点谁就切换谁那棵子树）。原来那条理由用两件事补上，而不是靠"不做"来回避：① 联动**可见**——所有受影响的复选框同帧重绘，半选一并刷新；② 计数与半选**按完整模型**算（过滤框只隐藏行，不改变模型），所以"看着勾了 3 个、实际同步 5 个"不会发生。「连同子部门」按钮随之删除（规则已是默认行为）。 |
| D16 半选与父行自身 | `indeterminate`（半选）的定义是**这一行与它自己的子树状态不一致**，两个方向都算：勾了父部门又单独取消某个子部门（"我把它排除了"），或只勾了子部门而父部门未勾（D9 的"为层级补建"）。**半选只是显示，不进接口**：提交给服务端的始终是"勾选中的行"组成的显式 id 集合（不是 `descendants:true` 之类的旗标），所以预览的统计与同步的写入描述的永远是同一批部门。**取消子部门不会取消父行**：父行的节点与它自己的人员仍在范围内——"所有子部门都没了就把父行也取消"是另一条规则，本轮不做。 |

**接口形状**：

- `GET /admin/api/v1/org/feishu/directory?departments=od_a,od_b`（可重复；缺省 = 全量）。
  响应里每个部门多出 `selected` 与 `included`（`included = selected ∪ 为层级补建`），
  每个人多出 `in_scope`；`stats` / `users_matched` / `memberships_to_add` 等**只统计范围内的对象**。
- `POST /admin/api/v1/org/feishu/sync`，body `{"department_ids":["od_a","od_b"]}`。
  响应在原有字段之外给出 `stats.departments_ancestors`、`unknown_department_ids`。

**为什么预览也要带选择**：M70 的核心不变量是"界面说会做什么，同步就做什么"（预览与同步共用
`planFeishuOrg`）。如果只在客户端过滤预览，统计数字与"哪些人会被写"就会悄悄偏离服务端的实际行为
（尤其是祖先补建与人员范围这两条）。
