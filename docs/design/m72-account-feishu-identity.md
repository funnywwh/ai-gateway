...
> 状态：**已实现（M72，代码与自动化测试完成；真机验收见 §11，待执行）**。
> 实现提交：`181586c`（设计+规格）、`4bcbde1`（票据）、`8dc558e`（迁移与配置）、`758164e`（aigw 判定与接口）、
> `1d225c9`（门户选择页）、`d9472a8`（控制台）。
...
1. **Key 登录的选择页票据由门户自己签**。设计（D4）设想"aigw 签、dshgw 验"的一条票据通道；实现下来
   两条路径各有来源：飞书登录仍由 aigw 用**登录票据同一个 codec** 的 `IssueKeyPick` 签（`feishu.pick_ttl_s`
   只影响它写下的 cookie 寿命），而 Key 登录的选择页是"门户自己受理、自己提问"，所以由门户的
   `feishu.Verifier.SignPick` 签（同一密钥、同一 `feishu-ticket` MAC 格式、mode 仍是 `keypick`）。
   原因：Key 登录根本没有 aigw 参与身份的环节，为它加一条 aigw 往返只会多一次可能失败的调用；
   而这两种票据在验证侧完全同构（`VerifyPick` / `PeekPick`），因此没有引入第二套格式。
   代价：aigw 侧的 `FeishuDeps.PickTickets` 被删掉了（它曾是第二份 codec），少一个要维护的配置。
2. **选择页 GET 不消费票据**：设计只写"一次性"。实现分成 `PeekPick`（GET 渲染）与 `VerifyPick`
   （POST 提交）——否则刷新一次表单就作废，浏览器预取链接也会把票据烧掉。契约向量与两侧测试都覆盖两者。
3. **Key 级绑定入口回答 400 `unsupported_parameter`，不是 410**：仓库已有"请求被理解但本部署不提供"
   的统一形状（同 M70 的"未启用飞书"），新增一个 410 只会多一种要客户端学的码。
4. **改绑会释放旧身份，而不是 409**。设计（D8）沿用了 M70 人员优先路由的"已绑别人 → 409"；实现中发现
   那条规则对账号优先的入口是错的：管理员在人员列表里改指另一个人时，旧身份必须被释放，否则
   `accounts.feishu_open_id` 的唯一索引会让"改绑"永远失败。现在：同一身份重复绑定是幂等（`result=bound`），
   换人是 `result=replaced`（先释放旧身份），而**新身份已被别的账号占用**仍然 409。
   释放动作自己写一条审计（`feishu_unbind`，reason=`replaced by another binding`）。
5. **authorize 响应多了一个 `account_id`**（设计里没有）：门户要把"我服务的租户"和"票据里的账号"对上，
   而票据按设计不带租户。账号 id 不是秘密（调用方已持有该账号的一把 Key），比让门户解析 worker Key
   名字可靠得多。
6. **模型预检只在登录路径做**。设计把它写在 `provisionAccountDSH` 里；实现发现那会让控制台的手动
   「启用 DSH」也拒绝——而那个操作者恰恰是能修模型授权的人，让他看到 dshgw 的原始措辞并无不可。
   于是检查移进 `ensureAccountDSHForLogin`（登录路径），控制台路径保持原样。
7. **`dsh_disabled_at` 每次「停用 DSH」都写**（含本来就没启用过的账号），而设计的 D2 只描述了状态机。
   理由：`auto_enable` 开着时，"停用"这个动作本身就是"我不同意自动启用"的表达，写下来才能兑现。
   同时每次停用都写审计（`changed` 字段说明是否真的翻了开关），否则"为什么它没被自动启用"将无法解释。
8. **M70 的 Key 级匹配通道被整条删除，没有留只读兜底**（设计 D9 写的是"删除"，这一条与实现一致；
   记录在差异里是因为它删掉了 `planFeishuOrg` 的一个入参与 `internal/webui` 里的一个标签）：
   一并改了 `admin_org_feishu_test.go` 的六处断言与 `org_feishu.js` 的通道标签表。
9. **控制台不再回显绑定结果码**：M60 那套 `#/keys?feishu=bound` 的回显逻辑与 11 个结果码随绑定入口一起
   删除（没有流程会再产生它们）。Key 页的 `render()` 因此不再接收 `route`/`navigate`；
   老的带 hash 的书签只会安静地渲染页面，`keys_feishu_test.mjs` 里新增了一条断言守着"不再报结果码"。
10. **`INPUT_MODES` 与 Key 的三个动作抽到 `pages/key_actions.js`**：组织页与 Keys 页共用。连带影响是
    `internal/webui/embed_test.go` 的"录制模式与后端一致"漂移检查改为读 `key_actions.js` + `keys.js`，
    `tags_binding_test.mjs` 也改钉共用模块（标签写入路径在那里）。
11. **组织页人员行的形态**：`personRow` 建 `.org-member` 摘要行（勾选框/姓名/id 的既有 CSS 规则与
    org_tree 测试的断言都还对得上），外面套一层可展开的 `.org-person` 容器。过滤态下**只禁止"加入"**
    （未勾选的行禁用），已勾选的行仍可取消——设计只写了"过滤时不可编辑"，实现把它收窄成"不可新增"，
    否则一次过滤就让成员关系变成只读。
12. **`GET /org/nodes/{id}/accounts` 每个账号多一次 `ListAPIKeys`**：人员行要显示 Key 计数（`active/total`），
    而账号列表接口一次读全表。当前规模（几十个账号）下这是同一次请求里的几十次本地读，代价可忽略；
    账号数上千时应改成一次聚合查询——记在这里，避免下一个人以为它是免费的。
13. **迁移每次启动都跑**（设计只说"启动时"）：实现是幂等的（Key 行迁移后已清空），所以每次启动都是
    一次廉价的空查询；没有引入"迁移标记表"是因为那会带来一个新的、可被误删的状态。
