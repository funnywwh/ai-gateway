...
>
> **2026-09-22 追补**：本文里的 `identity_source`（一把共享密钥灌给所有账号）**已删除**。本机曾把它
> 指向部署账号自己的 `~/.ssh/id_rsa`，8 个租户各拿到一份宿主私钥副本，而该密钥就在本机
> `authorized_keys` 里 ⇒ 租户可从沙箱 ssh 回宿主成为部署账号。现在唯一的预置来源是
